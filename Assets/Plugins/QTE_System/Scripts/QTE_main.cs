...
public class QTE_main : QTE_BaseClass
{
	...
	[HideInInspector]
	public int TouchType2;
	[HideInInspector]
	public int TouchType3;
	[HideInInspector]
	public int TouchType4;*/
	...
	private float TouchHoldCounter;
	private bool TouchHoldToggle;*/

    //private float SwipefingerStartTime = 0.0f;
    //private Vector2 SwipefingerStartPos = Vector2.zero;
    ...
    //These functions return the Timer Percentages normalized (0-1) for each button timer, used to Visalize the Timer onscreen.
    ...
    /*
     * public float GetTouchHoldTimerPercentage(){
		float myreturn = TouchHoldCounter/ TouchHoldTime;
		//DebugText.text = myreturn.ToString ();
		return 1 - myreturn;
	}*/
    ...
    public void TriggerQTE(float time, float delay, float TimerDelay)
    {
        ...
        if (IsTouch)
        {
            /*if (TouchType == 2)
                {
                    TouchHoldCounter = TouchHoldTime;
    ...
    private void ToggleAdditionalButtons(bool flip)
    {
        if (flip == true)
        {
            if (DualTrigger || TriTrigger || QuadTrigger)
    ...
    void PressButton()
    {
        //Debug.Log(ButtonIsAxisCheck[0] + " ," + ButtonIsAxisCheck[1]);
        //Debug.Log("Press Button");
        if (Input.GetButtonDown(KeyPress) && UIButtons[0].gameObject.activeSelf)
        {
            if (!ButtonIsAxisCheck[0]){ // Debug.Log("Button 1 succeeded");
                succeeded = true;
                KeyGotPressed = true;
    ...
    {
        //Only Run if the button is actually an axis
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (UIButtons[0].gameObject.activeSelf)
                        {
                            // Debug.Log("Something got pressed");
                            //if using Unity's predefined Inputs
                            if (UseInputs)
                            {
